...
${Object.entries(memo.memo_content?.sections || {}).map(([key, content]) => 
  `${key.replace(/_/g, ' ').toUpperCase()}\n${'-'.repeat(30)}\n${content}\n\n`
).join('')}

Generated by ReubenAI Investment Platform
  `;
  
  // Convert to base64 for download as text file
  return {
    url: `data:text/plain;base64,${btoa(simpleTextContent)}`,
    fileName: `IC_Memo_${deal.company_name}_${new Date().toISOString().split('T')[0]}.txt`
  };
}
