...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🎯 IC Datapoint Sourcing (Public) - Starting analysis for deal: ${deal_id}, manual: ${manual_trigger}`);

    // No authentication required for public function
    ...
    const icMemoContent = contentResults.reduce((acc, result) => ({
      ...acc,
      ...result
    }), {});
    ...
    await supabaseClient
      ...
      .insert({
        ...
        title: 'AI-Powered IC Analysis Generated (Public)',
        description: `IC memo sections generated using GPT-4o-mini AI analysis${manual_trigger ? ' (manual trigger)' : ' (automatic trigger)'} via public endpoint`,
        context_data: {
    ...
    const response = {
      ...
      memo_generation: {
        success: true,
        sections_generated: Object.keys(icMemoContent).length,
        word_count: Object.values(icMemoContent).join(' ').split(' ').length,
        error: null,
        ai_powered: true,
        model_used: 'gpt-4o-mini'
      },
      message: `Successfully generated ${Object.keys(icMemoContent).length} IC memo sections using AI analysis`
    };

    console.log(`🎉 IC Datapoint Sourcing (Public) completed successfully for deal: ${deal_id}`);
    ...
    console.error('❌ Error in AI-Powered IC Datapoint Sourcing (Public):', error);
    
    return new Response(JSON.stringify({
      success: false,
      deal_id: req.body?.deal_id || 'unknown',
      ...
      message: `IC Datapoint Sourcing failed: ${error.message}`,
      error: error.message
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
