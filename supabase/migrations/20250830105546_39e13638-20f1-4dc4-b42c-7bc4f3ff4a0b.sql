...
BEGIN
  ...
  -- Check if we already have a VC datapoint record for this deal
  SELECT id INTO existing_vc_datapoint_id
  FROM deal_analysis_datapoints_vc 
  WHERE deal_id = target_deal_id 
  LIMIT 1;
  
  -- Extract Employee Count (Waterfall Priority) - store as text
  ...
  IF linkedin_data.id IS NOT NULL AND NOT is_missing_value(linkedin_data.employees_in_linkedin) THEN
    extracted_employee_count := linkedin_data.employees_in_linkedin::text;
  ...
  IF extracted_employee_count IS NULL AND crunchbase_data.id IS NOT NULL AND NOT is_missing_value(crunchbase_data.num_employees) THEN
    extracted_employee_count := crunchbase_data.num_employees;
  ...
  IF extracted_employee_count IS NULL AND vc_datapoints_data.id IS NOT NULL THEN
    ...
    BEGIN
      ...
      IF linkedin_stored IS NOT NULL AND NOT is_missing_value(linkedin_stored::text) THEN
        extracted_employee_count := linkedin_stored::text;
      ...
      ELSE
        ...
        IF crunchbase_stored IS NOT NULL AND NOT is_missing_value(crunchbase_stored::text) THEN
          extracted_employee_count := crunchbase_stored::text;
  ...
  -- Priority 1: LinkedIn Export - founded
  ...
  -- Update existing record or insert new one without triggering sync triggers
  IF existing_vc_datapoint_id IS NOT NULL THEN
    -- Disable triggers temporarily for this session
    SET session_replication_role = replica;
    
    -- Update existing record
    UPDATE deal_analysis_datapoints_vc SET
      employee_count = COALESCE(extracted_employee_count, employee_count),
      founding_year = COALESCE(extracted_founding_year, founding_year),
      competitors = CASE WHEN extracted_competitors IS NOT NULL THEN ARRAY[extracted_competitors] ELSE competitors END,
      business_model = COALESCE(extracted_business_model, business_model),
      source_engines = ARRAY[employee_source, founding_source, competitors_source, business_model_source],
      ...
      END,
      updated_at = now()
    WHERE id = existing_vc_datapoint_id;
...
DROP TRIGGER IF EXISTS waterfall_extraction_crunchbase ON deal_enrichment_crunchbase_export;
CREATE TRIGGER waterfall_extraction_crunchbase
  AFTER INSERT OR UPDATE ON deal_enrichment_crunchbase_export
  FOR EACH ROW
  EXECUTE FUNCTION trigger_waterfall_extraction();

DROP TRIGGER IF EXISTS waterfall_extraction_linkedin ON deal_enrichment_linkedin_export;
CREATE TRIGGER waterfall_extraction_linkedin
  AFTER INSERT OR UPDATE ON deal_enrichment_linkedin_export
  FOR EACH ROW
  EXECUTE FUNCTION trigger_waterfall_extraction();

DROP TRIGGER IF EXISTS waterfall_extraction_linkedin_profile ON deal_enrichment_linkedin_profile_export;
CREATE TRIGGER waterfall_extraction_linkedin_profile
  AFTER INSERT OR UPDATE ON deal_enrichment_linkedin_profile_export
  FOR EACH ROW
  EXECUTE FUNCTION trigger_waterfall_extraction();

DROP TRIGGER IF EXISTS waterfall_extraction_perplexity_company ON deal_enrichment_perplexity_company_export_vc;
CREATE TRIGGER waterfall_extraction_perplexity_company
  AFTER INSERT OR UPDATE ON deal_enrichment_perplexity_company_export_vc
  FOR EACH ROW
  EXECUTE FUNCTION trigger_waterfall_extraction();

DROP TRIGGER IF EXISTS waterfall_extraction_perplexity_founder ON deal_enrichment_perplexity_founder_export_vc;
CREATE TRIGGER waterfall_extraction_perplexity_founder
  AFTER INSERT OR UPDATE ON deal_enrichment_perplexity_founder_export_vc
  FOR EACH ROW
  EXECUTE FUNCTION trigger_waterfall_extraction();

DROP TRIGGER IF EXISTS waterfall_extraction_perplexity_market ON deal_enrichment_perplexity_market_export_vc;
CREATE TRIGGER waterfall_extraction_perplexity_market
...
DROP TRIGGER IF EXISTS waterfall_extraction_datapoints_json ON deal_analysis_datapoints_vc;
CREATE TRIGGER waterfall_extraction_datapoints_json
  AFTER UPDATE ON deal_analysis_datapoints_vc
  FOR EACH ROW
  EXECUTE FUNCTION trigger_waterfall_extraction_datapoints();

-- Create function to backfill existing data with proper trigger handling
...
BEGIN
  -- Process all deals that have enrichment data
  FOR deal_record IN
    ...
    LIMIT 5 -- Process in smaller batches to avoid timeout
  LOOP
...
-- Run backfill for existing data (limited batch first)
