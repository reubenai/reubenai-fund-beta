-- Update the capture_deal_decision_memory trigger function to use correct memory_type
CREATE OR REPLACE FUNCTION public.capture_deal_decision_memory()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO public.fund_memory_entries (
      ...
      CASE 
        WHEN NEW.status IN ('approved', 'rejected') THEN 90
