-- Fix VC trigger conflicts: Drop existing triggers and create unified solution

-- Drop any existing conflicting triggers on deals table
DROP TRIGGER IF EXISTS trigger_perplexity_datamining_vc_on_deal_insert ON public.deals;
DROP TRIGGER IF EXISTS trigger_perplexity_vc_research_on_insert ON public.deals;

-- Create unified trigger function that both inserts records AND calls edge function
...
BEGIN
  ...
  IF fund_type_value = 'venture_capital' OR fund_type_value = 'vc' THEN
    ...
    -- Step 1: Insert record into perplexity_datamining_vc table
...
-- Create the trigger on deals table for INSERT operations
CREATE TRIGGER unified_vc_trigger_on_deal_insert
  AFTER INSERT ON public.deals
  FOR EACH ROW
  EXECUTE FUNCTION public.unified_vc_trigger_on_deal_creation();

-- Clean up any orphaned queued records to test fresh
DELETE FROM public.perplexity_datamining_vc 
