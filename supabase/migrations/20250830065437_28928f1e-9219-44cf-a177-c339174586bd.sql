...
WHERE f.fund_type = 'venture_capital'
  ...
  AND (
    -- Has Crunchbase data
    d.id IN (SELECT deal_id FROM deal_enrichment_crunchbase_export WHERE raw_brightdata_response IS NOT NULL) OR
    -- Has LinkedIn data  
    d.id IN (SELECT deal_id FROM deal_enrichment_linkedin_export WHERE raw_brightdata_response IS NOT NULL) OR
    -- Has Perplexity Company data
    d.id IN (SELECT deal_id FROM deal_enrichment_perplexity_company_export_vc WHERE raw_perplexity_response IS NOT NULL) OR
    -- Has Perplexity Founder data
    d.id IN (SELECT deal_id FROM deal_enrichment_perplexity_founder_export_vc WHERE raw_perplexity_response IS NOT NULL) OR
    -- Has Perplexity Market data
    d.id IN (SELECT deal_id FROM deal_enrichment_perplexity_market_export_vc WHERE raw_perplexity_response IS NOT NULL)
  );
...
UPDATE public.deal_analysis_datapoints_vc
SET 
  deal_enrichment_crunchbase_export = cbe.raw_brightdata_response,
  source_engines = CASE 
...
WHERE deal_analysis_datapoints_vc.deal_id = cbe.deal_id
  AND cbe.raw_brightdata_response IS NOT NULL;
