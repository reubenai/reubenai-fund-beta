...
BEGIN
  ...
  IF linkedin_data.id IS NOT NULL AND NOT is_missing_value(linkedin_data.raw_linkedin_response->>'employees_in_linkedin') THEN
    BEGIN
      extracted_employee_count := (linkedin_data.raw_linkedin_response->>'employees_in_linkedin')::integer;
  ...
  IF extracted_employee_count IS NULL AND crunchbase_data.id IS NOT NULL AND NOT is_missing_value(crunchbase_data.raw_crunchbase_response->>'num_employees') THEN
    DECLARE
      cb_employees text := crunchbase_data.raw_crunchbase_response->>'num_employees';
      match_result text;
    BEGIN
  ...
  IF linkedin_data.id IS NOT NULL AND NOT is_missing_value(linkedin_data.raw_linkedin_response->>'founded') THEN
    BEGIN
      extracted_founding_year := (linkedin_data.raw_linkedin_response->>'founded')::integer;
  ...
  IF extracted_founding_year IS NULL AND crunchbase_data.id IS NOT NULL AND NOT is_missing_value(crunchbase_data.raw_crunchbase_response->>'founded_date') THEN
    BEGIN
      extracted_founding_year := EXTRACT(YEAR FROM (crunchbase_data.raw_crunchbase_response->>'founded_date')::date);
  ...
  IF linkedin_data.id IS NOT NULL AND NOT is_missing_value(linkedin_data.raw_linkedin_response->>'similar_companies') THEN
    DECLARE
      similar_companies jsonb := linkedin_data.raw_linkedin_response->'similar_companies';
    BEGIN
  ...
  IF extracted_competitors IS NULL AND perplexity_company_data.id IS NOT NULL AND NOT is_missing_value(perplexity_company_data.raw_perplexity_response->>'key_market_players') THEN
    DECLARE
  ...
  IF extracted_business_model IS NULL AND perplexity_company_data.id IS NOT NULL AND NOT is_missing_value(perplexity_company_data.raw_perplexity_response->>'business_model') THEN
