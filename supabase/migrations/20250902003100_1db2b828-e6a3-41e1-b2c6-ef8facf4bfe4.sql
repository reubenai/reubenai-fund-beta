-- Complete VC Scoring Trigger Rebuild - Direct Execution (No Queue)

-- Phase 1: Clean slate - drop existing function and any triggers
DROP FUNCTION IF EXISTS public.trigger_vc_scoring_on_market_completion() CASCADE;

-- Phase 2: Create new direct trigger function (no queue, immediate execution)
...
BEGIN
  ...
  IF OLD.processing_status IS DISTINCT FROM NEW.processing_status AND NEW.processing_status = 'processed' THEN
    ...
    -- Create/update row in deal_analysisresult_vc with 'processing' status (skip queue)
    ...
    VALUES (
      ...
      0,
      'processing', -- Direct processing, no queue
      now(),
      now()
    )
    ...
    RAISE LOG '✅ Set deal_analysisresult_vc to processing status for deal %', NEW.deal_id;
...
-- Phase 3: Create the actual trigger on deal_enrichment_perplexity_market_export_vc
CREATE TRIGGER auto_trigger_vc_scoring_direct
...
RAISE LOG '🎯 Direct VC scoring trigger created successfully - no queue bottlenecks!';