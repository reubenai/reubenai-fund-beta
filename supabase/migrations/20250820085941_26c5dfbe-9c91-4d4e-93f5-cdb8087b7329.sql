-- Emergency Platform Restoration Phase 1 - FIXED JSON FORMAT
-- Fix the orchestrator infinite loop catastrophe

-- 1. Kill all running orchestrator executions (use correct JSON format)
UPDATE orchestrator_executions 
SET step_status = 'emergency_stopped',
    error_details = '{"emergency": true, "reason": "Emergency restoration - orchestrator loop prevention", "timestamp": "' || now()::text || '"}'::jsonb,
    updated_at = now()
WHERE step_status = 'running' OR step_status = 'processing';

-- 2. Clear stalled analysis queue items (set to emergency_paused to prevent restart)
...
-- 3. Create emergency ops control table
...
-- Enable RLS on emergency controls
ALTER TABLE emergency_ops_control ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Only Reuben admins can manage emergency controls
...
-- 4. Insert critical emergency controls with proper values
INSERT INTO emergency_ops_control (control_key, control_value, enabled, description) VALUES
('orchestrator_enabled', 'false', false, 'Global orchestrator kill switch - EMERGENCY'),
('auto_analysis_enabled', 'false', false, 'Auto analysis kill switch - EMERGENCY'),
('max_concurrent_orchestrations', '1', true, 'Emergency limit to prevent loops'),
('orchestrator_circuit_breaker', 'true', true, 'Emergency circuit breaker active'),
('cost_explosion_detected', 'true', true, 'Cost explosion prevention active')
