...
-- Step 2: Log the bulk cancellation action in activity_events
...
-- Step 3: Verification query (this will show in migration output)
SELECT 
  'Cancellation Summary' as operation,
  COUNT(*) as total_cancelled,
  MIN(created_at) as earliest_item,
  MAX(created_at) as latest_item,
  array_agg(DISTINCT trigger_reason) as trigger_reasons_cancelled
FROM analysis_queue 
WHERE status = 'cancelled' 
  AND error_message LIKE '%emergency system stabilization%';