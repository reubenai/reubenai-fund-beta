-- Phase 7: Admin Visibility & RLS Hardening - Complete Implementation
-- Step 1.1: Check current problematic policies (for verification)
CREATE OR REPLACE VIEW temp_problematic_policies AS
...
-- Step 1.2: Replace all dangerous RLS policies with pure-predicate templates

-- Organizations table - use id as both org identifier and org_id  
...
-- Add organization_id column to deals for direct org scoping (derived from fund)
...
SELECT
  (SELECT count(*) FROM public.organizations WHERE COALESCE(is_active, true) = true) as orgs_active,
  ...
  (SELECT count(*) FROM public.deals WHERE status IN ('sourced', 'screening', 'ic', 'active')) as deals_pipeline;
...
AS $$
  SELECT * FROM public.organizations WHERE (is_deleted IS NULL OR is_deleted = false);
$$;
...
-- Clean up temp view
DROP VIEW IF EXISTS temp_problematic_policies;