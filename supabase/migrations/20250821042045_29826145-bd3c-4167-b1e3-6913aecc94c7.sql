...
-- Define the engine types that need to be marked as completed
...
ON CONFLICT (deal_id, engine_name, source_type) DO UPDATE SET
  data_retrieved = EXCLUDED.data_retrieved,
  confidence_score = 100,
  validated = true,
  retrieved_at = now(),
  validation_notes = 'Emergency completion - analysis blocked';

-- Log the completion marking
...
SELECT 
  ...
  'emergency_shutdown',
  'critical',
  'All Engines Marked Complete',
  'Emergency: All analysis engines marked as completed to stop excessive re-analysis',
  jsonb_build_object(
    'engines_marked_complete', 13,
    'method', 'fake_completion_records',
    'reason', 'excessive_analysis_prevention'
  )
FROM deals d 
