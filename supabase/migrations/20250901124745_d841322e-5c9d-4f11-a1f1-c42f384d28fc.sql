-- Fix the trigger function to only check for 'venture_capital' (removing invalid 'vc' check)
...
BEGIN
  ...
  RAISE LOG 'Deal creation trigger: deal_id=%, fund_type=%, company=%', NEW.id, fund_type_value, NEW.company_name;
  
  -- Only process venture capital funds (FIXED: removed invalid 'vc' check)
  IF fund_type_value = 'venture_capital' THEN
    ...
    RAISE LOG 'Successfully inserted VC research record for deal: %', NEW.id;
    
    -- Step 2: Call the perplexity-datamining-vc edge function
    ...
    IF http_response.status = 200 THEN
      RAISE LOG 'Successfully triggered VC datamining for deal: %', NEW.id;
    ELSE
      RAISE LOG 'Failed to trigger VC datamining for deal: %. Status: %, Response: %', 
