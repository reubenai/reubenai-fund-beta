-- Fix HTTP response handling with correct field names
...
BEGIN
  ...
  IF OLD.processing_status IS DISTINCT FROM NEW.processing_status AND NEW.processing_status = 'processed' THEN
    ...
    BEGIN
      ...
      -- Use correct field names for PostgreSQL net.http_post response
      IF http_response.status = 200 THEN
        RAISE LOG '🎯 Successfully executed VC scoring engine for deal %. Response: %', NEW.deal_id, http_response.content;
      ELSE
        RAISE LOG '❌ VC scoring engine call failed for deal %. Status: %, Content: %', 
...
-- Test the final trigger one more time
