...
DECLARE
  target_fund_id uuid;
  target_org_id uuid;
  mapped_status processing_status;
BEGIN
  ...
  mapped_status := CASE 
    WHEN NEW.processing_status IN ('completed', 'processed') THEN 'completed'::processing_status
    ELSE 'failed'::processing_status
  END;
...
SELECT 
  ...
  CASE 
    WHEN pme.processing_status IN ('completed', 'processed') THEN 'completed'::processing_status
    ELSE 'failed'::processing_status
  END,
  NOW(),
  NOW(),
