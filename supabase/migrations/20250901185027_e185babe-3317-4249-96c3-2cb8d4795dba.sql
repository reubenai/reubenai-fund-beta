...
BEGIN
  ...
  completeness_percent := (completeness_score * 100 / total_fields);
  ...
  ON CONFLICT (deal_id) DO UPDATE SET
    ...
    data_completeness_score = completeness_percent,
    confidence_level = confidence_level,
    updated_at = NOW();
  
  RAISE LOG 'Populated deal_datapoints_vc for deal % with % fields (% percent complete)', 
    NEW.deal_id, completeness_score, completeness_percent;
