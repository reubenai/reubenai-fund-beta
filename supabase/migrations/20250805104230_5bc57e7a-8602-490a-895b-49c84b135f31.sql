-- Fix pipeline stage ordering: Investment Committee before Due Diligence
-- Step 1: Swap positions for Investment Committee and Due Diligence across all funds

-- Update Investment Committee from position 3 to position 2
UPDATE public.pipeline_stages 
SET position = 2 
WHERE LOWER(name) LIKE '%investment%committee%' AND position = 3;

-- Update Due Diligence from position 2 to position 3  
UPDATE public.pipeline_stages 
SET position = 3 
WHERE LOWER(name) LIKE '%due%diligence%' AND position = 2;

-- Step 2: Create function to generate default pipeline stages for new funds
...
-- Step 3: Update handle_new_fund trigger to create pipeline stages
...
-- Step 4: Create missing pipeline stages for existing funds that don't have them
DO $$
DECLARE
  fund_record RECORD;
BEGIN
...
-- Step 5: Ensure all existing funds have the correct stage order by name
-- This handles any funds with custom stage names but wrong positions
