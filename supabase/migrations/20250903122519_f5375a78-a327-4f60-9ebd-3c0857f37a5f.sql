-- Fix IC trigger to use correct field name status_code instead of status
...
BEGIN
  ...
  IF NEW.status = 'investment_committee' AND (OLD.status IS DISTINCT FROM 'investment_committee') THEN
    ...
    -- Log the HTTP response (FIXED: use status_code instead of status)
    IF http_response.status_code = 200 THEN
      RAISE LOG 'IC analysis successfully triggered for deal_id: %', NEW.id;
    ELSE
      RAISE LOG 'IC analysis trigger failed for deal_id: %. Status: %, Response: %', NEW.id, http_response.status_code, http_response.content;
    END IF;
    ...
    INSERT INTO public.activity_events (
      ...
      jsonb_build_object(
        ...
        'http_status', http_response.status_code,  -- FIXED: use status_code
