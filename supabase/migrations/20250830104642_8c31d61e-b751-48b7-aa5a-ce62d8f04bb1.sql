...
BEGIN
  ...
  -- Priority 1: LinkedIn Export - num_contacts_linkedin or num_employee_profiles
  IF linkedin_data.id IS NOT NULL AND NOT is_missing_value(linkedin_data.num_contacts_linkedin) THEN
    extracted_employee_count := linkedin_data.num_contacts_linkedin;
    employee_source := 'LinkedIn Export';
  ELSIF linkedin_data.id IS NOT NULL AND NOT is_missing_value(linkedin_data.num_employee_profiles) THEN
    extracted_employee_count := linkedin_data.num_employee_profiles;
    employee_source := 'LinkedIn Export';
  END IF;
  ...
  -- Priority 1: LinkedIn Export - check raw_brightdata_response for founded
  IF linkedin_data.id IS NOT NULL AND linkedin_data.raw_brightdata_response ? 'founded' AND NOT is_missing_value(linkedin_data.raw_brightdata_response->>'founded') THEN
    BEGIN
      extracted_founding_year := (linkedin_data.raw_brightdata_response->>'founded')::integer;
