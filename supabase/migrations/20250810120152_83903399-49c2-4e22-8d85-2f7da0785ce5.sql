...
-- B.1 Single source of truth for counts (view)
create or replace view dashboard_stats as
select
  (select count(*) from organizations where coalesce(is_test,false)=false and deleted_at is null) as orgs_active,
  (select count(*) from profiles where deleted_at is null) as users_total,
  (select count(*) from funds where is_active = true and deleted_at is null) as funds_active,
  (select count(*) from deals where status != 'rejected' and deleted_at is null) as deals_pipeline;
...
-- D.1 RLS linter views (keep recursion & gaps out forever)
