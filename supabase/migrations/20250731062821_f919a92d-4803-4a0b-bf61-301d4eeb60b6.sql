...
-- Create new RLS policies for deal_documents table using the helper function
CREATE POLICY "Users can view deal documents with new access" 
ON public.deal_documents 
FOR SELECT 
USING (public.has_document_management_access());

CREATE POLICY "Users can insert deal documents with new access" 
ON public.deal_documents 
FOR INSERT 
WITH CHECK (public.has_document_management_access());

CREATE POLICY "Users can update deal documents with new access" 
ON public.deal_documents 
FOR UPDATE 
USING (public.has_document_management_access());

CREATE POLICY "Users can delete deal documents with new access" 
ON public.deal_documents 
FOR DELETE 
USING (public.has_document_management_access());
