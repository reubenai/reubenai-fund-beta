-- Phase 1: Database Security Foundation (Fixed)
-- Drop existing policies first to avoid conflicts

-- Clean up existing policies for deal_notes
DROP POLICY IF EXISTS "Users can view deal notes for accessible deals" ON deal_notes;
DROP POLICY IF EXISTS "Users can manage deal notes for accessible deals" ON deal_notes;
DROP POLICY IF EXISTS "users_own_deal_notes" ON deal_notes;
DROP POLICY IF EXISTS "reuben_deal_notes_access" ON deal_notes;

-- Clean up existing policies for deal_documents
DROP POLICY IF EXISTS "Users can view deal documents for accessible deals" ON deal_documents;
DROP POLICY IF EXISTS "Users can insert sources for deals in their organization" ON deal_documents;
DROP POLICY IF EXISTS "Users can manage deal documents for accessible deals" ON deal_documents;

-- Clean up existing policies for ic_memos
DROP POLICY IF EXISTS "Users can manage IC memos with proper access" ON ic_memos;
DROP POLICY IF EXISTS "Users can view IC memos for accessible funds" ON ic_memos;

-- Clean up existing policies for investment_strategies
...
-- New RLS policies for deal_documents with proper restrictions
...
-- New RLS policies for deal_notes with proper creation restrictions
...
CREATE POLICY "Reuben admins have full access to notes" 
ON deal_notes FOR ALL 
USING (is_reuben_email())
WITH CHECK (is_reuben_email());

-- New RLS policies for IC memos with edit restrictions
...
-- New RLS policies for investment strategies
...
-- Fund creation policy to prevent analyst fund creation
