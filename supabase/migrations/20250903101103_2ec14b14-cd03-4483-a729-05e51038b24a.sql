-- Create trigger function to call IC analysis when deal status changes to 'Investment Committee'
...
BEGIN
  -- Only trigger when status changes TO 'Investment Committee'
  IF NEW.status = 'Investment Committee' AND (OLD.status IS DISTINCT FROM 'Investment Committee') THEN
...
-- Create the trigger on the deals table
DROP TRIGGER IF EXISTS deals_status_ic_trigger ON public.deals;
CREATE TRIGGER deals_status_ic_trigger
  AFTER UPDATE ON public.deals
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_ic_analysis_on_status_change();

-- Add comment for documentation
COMMENT ON FUNCTION public.trigger_ic_analysis_on_status_change() IS 'Automatically triggers IC analysis when a deal status changes to Investment Committee';
COMMENT ON TRIGGER deals_status_ic_trigger ON public.deals IS 'Triggers IC analysis when deal status changes to Investment Committee';