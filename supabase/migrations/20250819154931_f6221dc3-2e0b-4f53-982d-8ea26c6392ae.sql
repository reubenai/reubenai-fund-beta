        ...
        IF raw_data IS NOT NULL AND jsonb_array_length(raw_data) > 0 THEN
            ...
            SET 
                ...
                founded = CASE 
                    ...
                    ELSE NULL 
                END,
                team_size = COALESCE(first_record->'employees', '[]'::jsonb),
                headquarters = first_record->>'headquarters',
                industry = first_record->>'industries',
                description = first_record->>'about',
                specialties = specialties_array,
                linkedin_followers = CASE 
                    WHEN first_record->>'followers' ~ '^\d+$' 
                    THEN (first_record->>'followers')::INTEGER 
                    ELSE NULL 
                END,
                linkedin_updates = COALESCE(first_record->'updates', '[]'::jsonb),
                key_personnel = COALESCE(first_record->'employees', '[]'::jsonb),
