-- Phase 1: Fix Analysis Queue Execution
-- Create a comprehensive analysis function that calls all 5 specialist engines
...
BEGIN
  ...
  -- Get real engine results from deal_analysis_sources and ai_service_interactions
  SELECT 
    COALESCE(
      jsonb_agg(
        jsonb_build_object(
      ...
      ) FILTER (WHERE engine_name = 'market-intelligence-engine'),
      '[]'::jsonb
    ) INTO market_intelligence_data
  ...
  SELECT 
    COALESCE(
      ...
      ) FILTER (WHERE engine_name = 'financial-engine'),
      '[]'::jsonb
    ) INTO financial_analysis_data
  FROM deal_analysis_sources 
  ...
  SELECT 
    COALESCE(
      ...
      ) FILTER (WHERE engine_name = 'team-research-engine'),
      '[]'::jsonb
    ) INTO team_research_data
  FROM deal_analysis_sources 
  ...
  SELECT 
    COALESCE(
      ...
      ) FILTER (WHERE engine_name = 'product-ip-engine'),
      '[]'::jsonb
    ) INTO product_ip_data
  FROM deal_analysis_sources 
  ...
  SELECT 
    COALESCE(
      ...
      ) FILTER (WHERE engine_name = 'thesis-alignment-engine'),
      '[]'::jsonb
    ) INTO thesis_alignment_data
  FROM deal_analysis_sources 
  ...
  SELECT 
    jsonb_build_object(
      ...
      'trend_indicators', COALESCE(array_agg(DISTINCT tags) FILTER (WHERE tags IS NOT NULL), ARRAY[]::text[][]),
  ...
  real_insights := jsonb_build_object(
    'market_intelligence', 
    CASE 
      WHEN jsonb_array_length(market_intelligence_data) > 0 THEN 
        (market_intelligence_data->0->'data_retrieved')
    ...
    'financial_analysis',
    CASE 
      WHEN jsonb_array_length(financial_analysis_data) > 0 THEN 
        (financial_analysis_data->0->'data_retrieved')
      ELSE jsonb_build_object(
    ...
    'team_research',
    CASE 
      WHEN jsonb_array_length(team_research_data) > 0 THEN 
        (team_research_data->0->'data_retrieved')
      ELSE jsonb_build_object(
    ...
    'product_ip',
    CASE 
      WHEN jsonb_array_length(product_ip_data) > 0 THEN 
        (product_ip_data->0->'data_retrieved')
      ELSE jsonb_build_object(
    ...
    'thesis_alignment',
    CASE 
      WHEN jsonb_array_length(thesis_alignment_data) > 0 THEN 
        (thesis_alignment_data->0->'data_retrieved')
      ELSE jsonb_build_object(
  ...
  rubric_breakdown := jsonb_build_array(
    jsonb_build_object(
      ...
      'confidence', CASE WHEN jsonb_array_length(team_research_data) > 0 THEN 90 ELSE 50 END,
      'weight', 20,
      ...
      'strengths', CASE WHEN jsonb_array_length(team_research_data) > 0 
      ...
      'concerns', CASE WHEN jsonb_array_length(team_research_data) > 0 
    ...
    jsonb_build_object(
      ...
      'confidence', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 THEN 95 ELSE 50 END,
      'weight', 25,
      ...
      'strengths', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 
      ...
      'concerns', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 
    ...
    jsonb_build_object(
      ...
      'confidence', CASE WHEN jsonb_array_length(product_ip_data) > 0 THEN 90 ELSE 50 END,
      'weight', 25,
      ...
      'strengths', CASE WHEN jsonb_array_length(product_ip_data) > 0 
      ...
      'concerns', CASE WHEN jsonb_array_length(product_ip_data) > 0 
    ...
    jsonb_build_object(
      ...
      'confidence', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 THEN 90 ELSE 50 END,
      'weight', 15,
      ...
      'strengths', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 
      ...
      'concerns', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 
  ...
  analysis_engines := jsonb_build_object(
    'financial_engine', jsonb_build_object(
      ...
      'score', COALESCE(analysis_record.financial_score, 0),
      'confidence', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 THEN 90 ELSE 50 END,
      'status', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 THEN 'complete' ELSE 'pending' END,
      'last_run', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 
        THEN (financial_analysis_data->0->>'retrieved_at')::timestamp
        ELSE analysis_record.updated_at END,
      'version', '3.1',
      'data_sources', CASE WHEN jsonb_array_length(financial_analysis_data) > 0 THEN 'real_analysis' ELSE 'placeholder' END
    ),
    'market_intelligence', jsonb_build_object(
      'name', 'Market Intelligence Engine', 
      'score', COALESCE(analysis_record.market_score, 0),
      'confidence', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 THEN 95 ELSE 50 END,
      'status', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 THEN 'complete' ELSE 'pending' END,
      'last_run', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 
        THEN (market_intelligence_data->0->>'retrieved_at')::timestamp
        ELSE analysis_record.updated_at END,
      'version', '2.3',
      'data_sources', CASE WHEN jsonb_array_length(market_intelligence_data) > 0 THEN 'real_analysis' ELSE 'placeholder' END,
      'tam_sam_som_available', jsonb_array_length(market_intelligence_data) > 0
    ),
    'thesis_alignment', jsonb_build_object(
      'name', 'Thesis Alignment Engine',
      'score', COALESCE(analysis_record.thesis_alignment_score, 0), 
      'confidence', CASE WHEN jsonb_array_length(thesis_alignment_data) > 0 THEN 95 ELSE 50 END,
      'status', CASE WHEN jsonb_array_length(thesis_alignment_data) > 0 THEN 'complete' ELSE 'pending' END,
      'last_run', CASE WHEN jsonb_array_length(thesis_alignment_data) > 0 
        THEN (thesis_alignment_data->0->>'retrieved_at')::timestamp
        ELSE analysis_record.updated_at END,
      'version', '4.0',
      'data_sources', CASE WHEN jsonb_array_length(thesis_alignment_data) > 0 THEN 'real_analysis' ELSE 'placeholder' END
    ),
    'team_research', jsonb_build_object(
      'name', 'Team Research Engine',
      'score', COALESCE(analysis_record.leadership_score, 0),
      'confidence', CASE WHEN jsonb_array_length(team_research_data) > 0 THEN 90 ELSE 50 END,
      'status', CASE WHEN jsonb_array_length(team_research_data) > 0 THEN 'complete' ELSE 'pending' END,
      'last_run', CASE WHEN jsonb_array_length(team_research_data) > 0 
        THEN (team_research_data->0->>'retrieved_at')::timestamp
        ELSE analysis_record.updated_at END,
      'version', '2.8',
      'data_sources', CASE WHEN jsonb_array_length(team_research_data) > 0 THEN 'real_analysis' ELSE 'placeholder' END
    ),
    'product_ip', jsonb_build_object(
      'name', 'Product IP Engine',
      'score', COALESCE(analysis_record.product_score, 0),
      'confidence', CASE WHEN jsonb_array_length(product_ip_data) > 0 THEN 90 ELSE 50 END,
      'status', CASE WHEN jsonb_array_length(product_ip_data) > 0 THEN 'complete' ELSE 'pending' END,
      'last_run', CASE WHEN jsonb_array_length(product_ip_data) > 0 
        THEN (product_ip_data->0->>'retrieved_at')::timestamp
        ELSE analysis_record.updated_at END,
      'version', '2.5',
      'data_sources', CASE WHEN jsonb_array_length(product_ip_data) > 0 THEN 'real_analysis' ELSE 'placeholder' END
    )
  );
  ...
  enhanced_data := jsonb_build_object(
    ...
    'analysis_completeness', 
      CASE 
        WHEN analysis_record.overall_score IS NOT NULL THEN 
          CASE WHEN (
            jsonb_array_length(market_intelligence_data) > 0 AND
            jsonb_array_length(financial_analysis_data) > 0 AND
            jsonb_array_length(team_research_data) > 0 AND
            jsonb_array_length(product_ip_data) > 0 AND
            jsonb_array_length(thesis_alignment_data) > 0
          ) THEN 98 ELSE 75 END
        ELSE 60
      END,
    ...
    'fund_type_analysis', jsonb_build_object(
      ...
      'strengths', CASE WHEN (
        jsonb_array_length(market_intelligence_data) > 0 OR
        jsonb_array_length(financial_analysis_data) > 0 OR
        jsonb_array_length(team_research_data) > 0
      ...
      'strategic_recommendations', CASE WHEN (
        jsonb_array_length(market_intelligence_data) > 0 OR
        jsonb_array_length(financial_analysis_data) > 0
    ...
    'engines_completion_status', jsonb_build_object(
      'market_intelligence_complete', jsonb_array_length(market_intelligence_data) > 0,
      'financial_analysis_complete', jsonb_array_length(financial_analysis_data) > 0,
      'team_research_complete', jsonb_array_length(team_research_data) > 0,
      'product_ip_complete', jsonb_array_length(product_ip_data) > 0,
      'thesis_alignment_complete', jsonb_array_length(thesis_alignment_data) > 0,
      'total_engines_complete', (
        CASE WHEN jsonb_array_length(market_intelligence_data) > 0 THEN 1 ELSE 0 END +
        CASE WHEN jsonb_array_length(financial_analysis_data) > 0 THEN 1 ELSE 0 END +
        CASE WHEN jsonb_array_length(team_research_data) > 0 THEN 1 ELSE 0 END +
        CASE WHEN jsonb_array_length(product_ip_data) > 0 THEN 1 ELSE 0 END +
        CASE WHEN jsonb_array_length(thesis_alignment_data) > 0 THEN 1 ELSE 0 END
      ),
...
-- Phase 2: Update the existing populate_enhanced_analysis function to use real engines
DROP FUNCTION IF EXISTS populate_enhanced_analysis(uuid);
...
AS $$
BEGIN
  RETURN populate_enhanced_analysis_with_real_engines(target_deal_id);
END;
$$;