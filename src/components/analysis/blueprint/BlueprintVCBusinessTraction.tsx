...
export function BlueprintVCBusinessTraction({ deal }: BusinessTractionProps) {
  const [loading, setLoading] = useState(true);
  const [subCriteria, setSubCriteria] = useState<SubCriteriaScore[]>([]);
  const [expandedSections, setExpandedSections] = useState<string[]>([]);
  const [overallScore, setOverallScore] = useState(0);

  const tractionSubCriteria = [
    { id: 'revenue-growth', name: 'Revenue Growth', weight: 40, icon: <TrendingUp className="h-4 w-4" /> },
    { id: 'business-model-viability', name: 'Business Model Viability', weight: 35, icon: <DollarSign className="h-4 w-4" /> },
    { id: 'customer-traction', name: 'Customer Traction', weight: 25, icon: <Users className="h-4 w-4" /> }
  ];
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {subCriteria.map((criteria, index) => {
          const criteriaConfig = tractionSubCriteria[index];
                ...
                <div className="mt-3 space-y-3 bg-accent/20 p-3 rounded-lg">
                  ...
                  <div className="pt-2 border-t">
                    <p className="text-xs text-muted-foreground">
                      Analysis based on financial performance data, customer metrics, and business model evaluation.
                    </p>
                  </div>
                </div>
              </CollapsibleContent>
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg">
          <h4 className="font-medium mb-2">Business Traction Summary</h4>
          <p className="text-sm text-muted-foreground">
            Comprehensive traction analysis across {subCriteria.length} key criteria focusing on growth and business model validation.
            Overall traction strength: <span className={`font-medium ${getScoreColor(overallScore)}`}>{overallScore}%</span>
          </p>
        </div>
      </CardContent>
    </Card>
