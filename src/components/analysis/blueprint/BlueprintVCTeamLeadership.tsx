...
export function BlueprintVCTeamLeadership({ deal }: TeamLeadershipProps) {
  const [loading, setLoading] = useState(true);
  const [subCriteria, setSubCriteria] = useState<SubCriteriaScore[]>([]);
  const [expandedSections, setExpandedSections] = useState<string[]>([]);
  const [overallScore, setOverallScore] = useState(0);

  const teamSubCriteria = [
    { id: 'founder-market-fit', name: 'Founder-Market Fit', weight: 30, icon: <Users className="h-4 w-4" /> },
    { id: 'previous-experience', name: 'Previous Experience', weight: 25, icon: <Award className="h-4 w-4" /> },
    { id: 'technical-expertise', name: 'Technical Expertise', weight: 25, icon: <Code className="h-4 w-4" /> },
    { id: 'execution-track-record', name: 'Execution Track Record', weight: 20, icon: <Target className="h-4 w-4" /> }
  ];
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {subCriteria.map((criteria, index) => {
          const criteriaConfig = teamSubCriteria[index];
                ...
                <div className="mt-3 space-y-3 bg-accent/20 p-3 rounded-lg">
                  ...
                  <div className="pt-2 border-t">
                    <p className="text-xs text-muted-foreground">
                      Assessment based on LinkedIn profiles, team research, industry expertise analysis, and track record evaluation.
                    </p>
                  </div>
                </div>
              </CollapsibleContent>
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg">
          <h4 className="font-medium mb-2">Team & Leadership Summary</h4>
          <p className="text-sm text-muted-foreground">
            Comprehensive team analysis across {subCriteria.length} key criteria with emphasis on founder-market fit and execution capability.
            Overall team strength: <span className={`font-medium ${getScoreColor(overallScore)}`}>{overallScore}%</span>
          </p>
        </div>
      </CardContent>
    </Card>
