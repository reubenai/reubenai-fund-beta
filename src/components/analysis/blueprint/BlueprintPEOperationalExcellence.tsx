...
export function BlueprintPEOperationalExcellence({ deal }: OperationalExcellenceProps) {
  const [loading, setLoading] = useState(true);
  const [subCriteria, setSubCriteria] = useState<SubCriteriaScore[]>([]);
  const [expandedSections, setExpandedSections] = useState<string[]>([]);
  const [overallScore, setOverallScore] = useState(0);

  const operationalSubCriteria = [
    { id: 'management-team-strength', name: 'Management Team Strength', weight: 40, icon: <Users className="h-4 w-4" /> },
    { id: 'operational-efficiency', name: 'Operational Efficiency', weight: 35, icon: <Settings className="h-4 w-4" /> },
    { id: 'technology-systems', name: 'Technology & Systems', weight: 25, icon: <Laptop className="h-4 w-4" /> }
  ];
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {subCriteria.map((criteria, index) => {
          const criteriaConfig = operationalSubCriteria[index];
                ...
                <div className="mt-3 space-y-3 bg-accent/20 p-3 rounded-lg">
                  ...
                  <div className="pt-2 border-t">
                    <p className="text-xs text-muted-foreground">
                      Assessment covers management quality, operational KPIs, process efficiency, and technology infrastructure.
                    </p>
                  </div>
                </div>
              </CollapsibleContent>
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg">
          <h4 className="font-medium mb-2">Operational Excellence Summary</h4>
          <p className="text-sm text-muted-foreground">
            Comprehensive operational assessment across {subCriteria.length} key criteria focusing on management and efficiency.
            Overall operational excellence: <span className={`font-medium ${getScoreColor(overallScore)}`}>{overallScore}%</span>
          </p>
        </div>
      </CardContent>
    </Card>
